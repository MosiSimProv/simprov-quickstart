...
def process_executing_simulation_experiment(event):
    ...
    python_env = Agent("Python")
    python_env.attributes["Version"] = event["python_version"]
    python_env.attributes["Packages"] = event["python_packages"]

    simulator = Agent("Simulator")
    simulator.attributes["Version"] = event["simulator_version"]
    simulator.attributes["Formalism"] = event["simulator_formalism"]

    activity.generated_entities = generated_entities
    activity.used_entities = used_entities
    activity.associated_agents=  [python_env,simulator]
    return activity
...
def process_analyzing_simulation_data(event):
    ...
    activity.generated_entities = generated_entities
    activity.used_entities = used_entities
    activity.associated_agents = [python_env]
    return activity
